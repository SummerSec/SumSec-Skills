<!--
  本文件 = skill-optimizer 的【官方规范本地缓存】。
  - 路径：${CLAUDE_SKILL_DIR}/references/official-spec-fetch.md
  - 用法：当 WebFetch https://code.claude.com/docs/en/skills.md 失败时，读取本文件作为降级基准。
  - 维护：每次 WebFetch 成功后，整文件覆盖（保留本头部 + 抓取时间字段）。
  - 来源：https://code.claude.com/docs/en/skills.md
  - fetched_at: 2026-05-25T06:34:57Z
  - method: curl --max-time 20 (initial seed)
-->
